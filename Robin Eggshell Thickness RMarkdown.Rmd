...
```
A lot of eggs were not able to be seperated (yolk and albumen) effectively, or lost mass due to cracking prior to seperation. If we want to look at albumen or yolk mass, those eggs should be removed for those analyses. That dataset is shell_sep. 
```{r}
shell_sep <- shell %>% 
...
Redoing using only the normally shaped eggs
```{r}
mod_length2 <- lmer(Thickness_shell ~ Length_egg + (1|NestID), data=shell2)
plot(mod_length2)
summary(mod_length2)
anova(mod_length2)

```
There are still some real outliers 

```{r}
hist(shell2$Length_egg)
shell3 <- shell %>% filter(Length_egg>2.6)
hist(shell3$Length_egg)

mod_length3 <- lmer(Thickness_shell ~ Length_egg + (1|NestID), data=shell3)
plot(mod_length3)
summary(mod_length3)
anova(mod_length3)
...
ggplot()+
  ...
  geom_point(data=shell %>% filter(Length_egg<2.4), aes(x=Length_egg, y=Thickness_shell), color="red")+
  geom_smooth(data=shell, aes(x=Length_egg, y=Thickness_shell), method="lm", color="red")+
  geom_smooth(data=shell2, aes(x=Length_egg, y=Thickness_shell), color="black", method="lm")+
...
If we look at the full range of parasite loads, what does it look like?
```{r}
mod_ecto <- lmer(Thickness_shell ~ Ectoparasites+ Length_egg+(1|NestID), data=shell3)
plot(mod_ecto)
qqnorm(resid(mod_ecto))
summary(mod_ecto)
confint(mod_ecto)
...
Controlling for egg length
```{r}
mod_yolk <- lmer(Thickness_shell ~ Mass_yolk + Length_egg + (1|NestID), data=shell_sep)
plot(mod_yolk)
summary(mod_yolk)
confint(mod_yolk)
anova(mod_yolk)
...
```
When controlling for egg length, there is no relationship between yolk mass and eggshell thickness. 


Controlling for egg volume
```{r}
#Including everything
mod_yolk <- lmer(Thickness_shell~ Mass_yolk + Volume_egg  + (1|NestID), data=shell_sep) 
...
ggplot(shell_sep, aes(y=Thickness_shell, x=Mass_yolk ))+
  geom_point()+
  geom_smooth(method="lm")
...
Controlling for egg length
```{r}
mod_alb <- lmer(Thickness_shell ~Mass_albumen + Length_egg + (1|NestID), data=shell_sep)
plot(mod_alb)
summary(mod_alb)
confint(mod_alb)
anova(mod_alb)
...
Controlling for egg volume
```{r}
mod_alb <- lmer(Thickness_shell~Mass_albumen + Volume_egg + (1|NestID), data=shell_sep)
plot(mod_alb)
summary(mod_alb)
anova(mod_alb)
...
```{r}
PCA_color <- prcomp( shell[,c(39:41, 43:44)], 
               center=T, 
               scale=T, 
               retx=T)
...
hist(shell3$T_concentration)
median(shell3$T_concentration, na.rm=T)

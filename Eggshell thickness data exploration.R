...
mod_incstage <- lm(Thickness_shell~IncStage, data=shell2)
plot(mod_incstage)
...
cmod_alb <- lm(Mass_albumen~IncStage, data=shell2)
...
#Egg color 
mod_eggcolor <- lmer(Thickness_shell ~ HTheta_egg + HPhi_egg + RVec_egg + (1|NestID), data=shell2)
...
ggplot(shell2 %>% filter(HTheta_egg>2.15 & Thickness_shell<=0.14), aes(x=HTheta_egg, y=Thickness_shell))+
  geom_point(shape=1)+
  geom_smooth(method="lm")
